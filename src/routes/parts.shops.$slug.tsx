...
function ShopStorefront() {
  ...
  return (
    <main className="mx-auto max-w-[1180px] px-4 py-8 sm:px-6 lg:py-12">
      ...
      <section className="mt-5 rounded-3xl border border-border/80 bg-gradient-to-br from-teal-50 to-white p-7 shadow-card sm:p-10">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold text-foreground">
              {supplier?.name ?? "Supplier storefront"}
            </h1>
            <p className="mt-2 text-sm text-muted-foreground">
              {supplier?.city ? `${supplier.city}, ` : ""}
              {supplier?.country ?? ""} · Stated inventory, delivery and terms from this supplier.
            </p>
          </div>
        </div>
        {supplier && (
          <div className="mt-6 grid gap-3 text-sm sm:grid-cols-2">
            <p className="rounded-xl bg-white/10 p-3">
              <Truck className="mr-2 inline h-4 w-4" /> Ships to{" "}
              {supplier.shipping_regions?.join(", ") || "regions confirmed in quote"}
            </p>
            <p className="rounded-xl bg-white/10 p-3">
              Returns: {supplier.return_policy || "Confirm terms in quote"}
            </p>
          </div>
        )}
      </section>
      {inventory.isLoading ? (
        ...
        <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {inventory.data.map((part) => (
            ...
            >
              <p className="text-xs font-bold uppercase text-primary">{part.brand || "Parts"}</p>
              <h2 className="mt-2 text-lg font-bold">{part.title}</h2>
              <p className="mt-1 text-sm text-muted-foreground">
                {part.part_number
                  ? `Part no. ${part.part_number}`
                  : "Part number confirmed in quote"}
              </p>
              <p className="mt-5 text-xl font-extrabold">{money(part.price, part.currency)}</p>
              <p className="mt-1 text-xs text-muted-foreground">
                {part.stock_quantity} stated in stock · Warranty:{" "}
                {part.warranty_text || "confirm in quote"}
              </p>
              <p className="mt-2 text-xs text-muted-foreground">
                Returns: {part.return_policy || supplier?.return_policy || "confirm in quote"}
              </p>
              <Button asChild className="mt-5 w-full" size="sm">
                <Link to="/parts/$id" params={{ id: part.id }}>
                  Check fitment & request quote
                </Link>
              </Button>
            </article>
          ))}
        ...
        <EmptyState
          icon={<PackageSearch />}
          title="No published stock in this storefront"
          description="This supplier has not published live items yet."
        />
      )}
    </main>
  );
