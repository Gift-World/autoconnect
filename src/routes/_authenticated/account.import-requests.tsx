  ...
  return (
    <div className="space-y-3">
      {data.map((r) => {
        ...
        const source = r.preferred_source_country
          ? countryByCode(r.preferred_source_country)
          : null;
        return (
          <div key={r.id} className="rounded-xl border border-border bg-card p-5 shadow-sm">
            ...
            <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
              <span>
                Budget: {r.budget_min ?? "?"} – {r.budget_max ?? "?"} {r.budget_currency}
              </span>
              <span>
                Deliver to: {country?.flag} {country?.name ?? r.buyer_country}
              </span>
              {source && (
                <span>
                  From: {source.flag} {source.name}
                </span>
              )}
