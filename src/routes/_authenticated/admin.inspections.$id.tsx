...
function AdminInspectionDetail() {
  ...
  async function load() {
    ...
    setTyres(
      cl.tyres ?? { make: "", size: "", condition: "", spare_present: false, spare_condition: "" },
    );
  ...
  async function approve() {
    ...
    else toast.success("Approved. Buyers will see the Inspection Done badge.");
    load();
  }
  ...
  return (
    <div className="space-y-6">
            ...
            {s.items.map((it) => (
              <label key={it.key} className="flex items-center gap-2 rounded-md border p-2 text-sm">
                <Checkbox
                  ...
                  onCheckedChange={(v) =>
                    setChecklist((c) => ({ ...c, [`${s.key}.${it.key}`]: !!v }))
                  }
                />
                {it.label}
              </label>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Tyres</CardTitle>
        </CardHeader>
        <CardContent className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            ...
            <Input
              value={tyres.make}
              onChange={(e) => setTyres({ ...tyres, make: e.target.value })}
            />
          </div>
          <div>
            ...
            <Input
              value={tyres.size}
              onChange={(e) => setTyres({ ...tyres, size: e.target.value })}
            />
          </div>
          <div>
          ...
          <label className="flex items-center gap-2 pt-6 text-sm">
            <Checkbox
              checked={tyres.spare_present}
              onCheckedChange={(v) => setTyres({ ...tyres, spare_present: !!v })}
            />{" "}
            Spare tyre present
          </label>
          {tyres.spare_present && (
            <div className="sm:col-span-2">
              ...
              <Input
                value={tyres.spare_condition}
                onChange={(e) => setTyres({ ...tyres, spare_condition: e.target.value })}
              />
            </div>
          )}
        </CardContent>
        ...
        <CardContent className="space-y-3">
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Short summary shown to buyers."
              rows={3}
            />
          </div>
          <div>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button onClick={() => save("completed")} disabled={saving}>
          {saving && <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" />} Save & mark completed
        </Button>
      </div>
    </div>
  );
