  ...
  head: () => ({
    meta: [
      { title: "How Payments Work — AutoConnect" },
      {
        name: "description",
        content:
          "Payment protected: card, M-Pesa or bank transfer. Funds released after verification. See how AutoConnect handles payments, handover and disputes.",
      },
      { property: "og:title", content: "How Payments Work — AutoConnect" },
      {
        property: "og:description",
        content:
          "Payment protected, funds released after verification. Card, M-Pesa or bank transfer with admin review.",
      },
    ...
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
      <header className="text-center">
        ...
        <p className="mt-3 text-muted-foreground">
          AutoConnect holds the payment while documents are reviewed and the car changes hands.
          Funds are released after verification.
        </p>
      </header>

      <ol className="mt-12 space-y-6">
        <Step n={1} icon={<Lock />} title="Buyer pays or reserves">
          Pay by card through Stripe, or reserve with M-Pesa or a bank transfer. The amount is the
          car price plus a 5% service fee. Money is never sent straight to the seller.
        </Step>
        <Step n={2} icon={<Smartphone />} title="Manual payment reviewed by admin">
          M-Pesa and bank transfers are an MVP manual flow: you submit the payer name, phone and
          reference, and the transaction sits in review until AutoConnect confirms the funds landed.
          Card payments are confirmed automatically.
        </Step>
        <Step n={3} icon={<ShieldCheck />} title="Car moves to Under Transaction">
          Once payment is confirmed the listing is marked Under Transaction so nobody else can buy
          it, and documents are reviewed by AutoConnect.
        </Step>
        <Step n={4} icon={<Handshake />} title="Seller prepares handover">
          The seller gets the car, keys and paperwork ready and marks the handover as ready. You
        ...
        <Step n={6} icon={<Banknote />} title="Funds released after verification">
          AutoConnect releases the car price to the seller — via Stripe Connect (typically 2-5
          business days) or a manual payout for M-Pesa and bank deals. The 5% service fee covers
          verification, payment handling and dispute support.
        </Step>
        <Step n={7} icon={<AlertTriangle />} title="If there's a dispute…" subtle>
          You can raise a dispute before confirming receipt. Funds stay held while our team reviews
          the case.
        </Step>
        <Step n={8} icon={<RotateCcw />} title="Refunds" subtle>
          If a case is decided in the buyer's favour, AutoConnect returns the payment — to the card
          for Stripe payments, or to the M-Pesa/bank account used for manual payments. Outcomes
          depend on the review.
        </Step>
      </ol>
...
function Step({
  ...
  return (
    <li
      className={`flex gap-4 rounded-xl border p-5 ${subtle ? "bg-muted/30" : "bg-card shadow-sm"}`}
    >
