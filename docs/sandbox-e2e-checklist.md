...
Create four verified test identities: buyer, parts supplier, service provider, and administrator. Give the supplier one approved `parts_shops` record and the provider one approved `service_providers` record. Apply migrations `0022_connected_aftercare_hardening.sql` and `0023_aftercare_receipts_reviews.sql` before testing.

## Journeys to pass

1. Buyer adds a vehicle, requests a part quote, and links it to that vehicle. The supplier can see only its own inquiry.
2. Buyer requests service against its vehicle. The provider quotes it, buyer approves it, provider confirms, starts, and completes it. A receipt number is created; the buyer submits one review.
