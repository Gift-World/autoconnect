...
export const Route = createFileRoute("/api/public/service-providers")({ server: { handlers: { GET: async () => {
  ...
  const { data, error } = await supabaseAdmin.from("service_providers").select("id,name,provider_type,country,city,description,phone,is_verified").eq("is_approved", true).order("name");
