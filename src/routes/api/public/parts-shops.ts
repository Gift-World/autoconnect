...
export const Route = createFileRoute("/api/public/parts-shops")({ server: { handlers: { GET: async () => {
  ...
  const { data, error } = await supabaseAdmin.from("parts_shops").select("id,slug,name,description,country,city,shipping_regions,return_policy,is_verified,is_sample").eq("is_approved", true).eq("is_suspended", false).order("name");
