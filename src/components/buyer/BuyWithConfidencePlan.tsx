  ...
  const evidence: Evidence[] = [
    {
      ...
      detail:
        imageCount > 0
          ? `${imageCount} photo${imageCount === 1 ? "" : "s"} attached`
    ...
    <section className="rounded-2xl border border-primary/25 bg-gradient-to-br from-primary/10 via-card to-card p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">
            Buyer confidence plan
          </p>
          <h2 className="mt-1 text-lg font-bold">Know what to verify before you buy</h2>
          <p className="mt-1 max-w-xl text-sm text-muted-foreground">
            A plain-language checklist for {title}. AutoConnect only marks evidence complete when it
            exists on the listing.
          </p>
        </div>
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
        {evidence.map((item) => (
          <div
            key={item.label}
            className="flex items-start gap-2.5 rounded-xl border border-border/70 bg-background/70 p-3"
          >
            {item.ready ? (
      ...
      {!canProceed && (
        <p className="mt-3 flex items-center gap-1.5 text-xs text-muted-foreground">
          <FileSearch className="h-3.5 w-3.5" /> Do not release funds until the missing evidence is
          reviewed.
        </p>
      )}
    </section>
